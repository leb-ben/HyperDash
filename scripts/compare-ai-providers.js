/**
 * AI Provider Comparison Test (JavaScript version)
 * Compares Perplexity vs Cerebras for trading analysis
 * Run: node scripts/compare-ai-providers.js
 */
...
class AIProviderComparison {
  ...
  async testProvider(client, model, prompt, provider) {
  ...
  extractReasoning(response) {
    const hasReasoning = /because|since|due to|based on|technical|fundamental|rsi|macd|volume|price/gi.test(response);
  ...
  calculateScore(response) {
    ...
    // Confidence (20%)
    if (response.confidence > 0.7) score += 20;
    else if (response.confidence > 0.5) score += 15;
    else score += 10;
  ...
  async runComparison() {
    ...
    let perplexityResponses = [];
    let cerebrasResponses = [];

/**
 * 24-Hour Test Monitor - Windows Compatible
 * Monitors the aggressive trading test and provides real-time alerts
 * Run: node scripts/24hour-monitor-windows.js
 */
...
class WindowsTestMonitor {
  ...
  async startMonitoring() {
    console.log('🔍 Starting 24-Hour Trading Test Monitor (Windows)');
  ...
  async performHealthCheck() {
    ...
    try {
      ...
      // Check system resources (Windows compatible)
  ...
  checkSystemResources() {
    try {
      // Use Node.js os module for Windows compatibility
      const totalMem = os.totalmem();
      const freeMem = os.freemem();
      const memUsagePercent = Math.round(((totalMem - freeMem) / totalMem) * 100);
      
      // Check disk space using Node.js (simplified)
      const stats = fs.statSync('.');
      let diskUsagePercent = 50; // Default estimate
      
      // Try to get more accurate disk usage on Windows
      try {
        const drive = process.cwd().split(':')[0];
        const output = execSync(`wmic logicaldisk where "DeviceID='${drive}:'" get size,freespace /format:list`, { encoding: 'utf8' });
        ...
        if (sizeLine && freeLine) {
          ...
          diskUsagePercent = Math.round(((size - free) / size) * 100);
        }
      } catch (diskError) {
        // Fallback to estimate if wmic fails
        diskUsagePercent = 50;
      }
  ...
  displayStatus(hoursElapsed, hoursRemaining, botStatus, portfolio, systemStatus) {
    console.clear();
    console.log('📊 24-Hour Trading Test Monitor (Windows)');
    console.log('='.repeat(60));
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const monitor = new WindowsTestMonitor();
  monitor.startMonitoring().catch(console.error);
}

export default WindowsTestMonitor;

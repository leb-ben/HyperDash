...
interface OptimizationTest {
  ...
  reason?: string // NEW: Why was it adopted or reverted?
...
interface HamburgerBotConfig {
  ...
  ai?: { // Make optional to match imported type
    aggressiveness?: 'low' | 'conservative' | 'medium' | 'aggressive' | 'high' | 'ultra' // DEPRECATED
...
export class FastBacktestEngine {
  ...
  private lastDayMetrics: FastMetrics | null = null // Track for daily comparison
  ...
  constructor(config: HamburgerBotConfig) {
    ...
    this.peakEquity = config.totalInvestmentUsd
  ...
  private evaluateOptimizationTests(metrics: FastMetrics): void {
    for (const test of this.activeTests) {
      ...
      if (metrics.maxDrawdown > 20) {
        ...
        this.revertTest(test, 'High drawdown detected (>20%)')
      } else if (metrics.totalReturnPct < -5) {
        // Revert on losses
        this.revertTest(test, 'Negative returns detected (<-5%)')
      } else if (metrics.totalReturnPct > 5 && metrics.sharpeRatio > 1.5) {
        // Adopt if performance is solid
        test.status = 'adopted'
        test.endDate = new Date().toISOString()
        test.reason = `Solid performance improvement (${metrics.totalReturnPct.toFixed(2)}% return, ${metrics.sharpeRatio.toFixed(2)} Sharpe)`
        logger.info(`Adopted optimization: ${test.parameter} showed improvement (${metrics.totalReturnPct.toFixed(2)}% return, ${metrics.sharpeRatio.toFixed(2)} Sharpe)`)
      }
  ...
  async runBacktest(): Promise<{
    ...
    optimizationTests: OptimizationTest[]
    finalConfig: HamburgerBotConfig
  }> {
    ...
    for (let i = 50; i < this.data.length; i += batchSize) {
      ...
      for (let j = i; j < endIndex; j++) {
        ...
        const decision = this.makeDecision(j)
        ...
        // --- PERIODIC SELF-IMPROVEMENT CYCLE (Every 24 hours / 1440 candles) ---
        if (j > 0 && j % 1440 === 0) {
          const dailyMetrics = this.calculateMetrics()
          const report = this.generateDailyReport(dailyMetrics)
          this.dailyReports.push(report)
          
          // Re-evaluate existing tests
          this.evaluateOptimizationTests(dailyMetrics)
          
          // Apply new optimization based on yesterday's performance
          this.applyTopSuggestion(report)
          
          // Update baseline for tomorrow
          this.lastDayMetrics = dailyMetrics
          
          logger.info(`Learning Cycle Completed: Day ${j / 1440} | Current Equity: $${this.equity.toFixed(2)}`)
        }
      }
    }
    ...
    // Final report if not exactly on a 24h mark
    if (this.data.length % 1440 !== 0) {
    ...
    logger.info(`Learning Cycles: ${this.dailyReports.length} | Improvements Applied: ${this.activeTests.length}`)
    logger.info('================================')
    ...
    return {
      ...
      optimizationTests: this.activeTests,
   ...
   */
  private makeDecision(index: number): AIDecision {
    ...
    this.updateSignalPerformance(signals, index)
   ...
   */
  private updateSignalPerformance(signals: GridSignals, index: number): void {
    ...
    // Look ahead 5 candles to see if signal was "correct"
    if (index + 5 >= this.data.length) return;
    
    const currentPrice = this.data[index]?.close || 0;
    const futurePrice = this.data[index + 5]?.close || 0;
    const priceChange = ((futurePrice - currentPrice) / currentPrice) * 100;

    // 1. Parabolic SAR Performance
    this.signalPerformance.parabolicSAR.total++;
    if ((signals.parabolicSAR.isUptrend && priceChange > 0.1) || 
        (!signals.parabolicSAR.isUptrend && priceChange < -0.1)) {
      this.signalPerformance.parabolicSAR.correct++;
    }

    // 2. Volume Spike Performance
    if (signals.volume.isSpike) {
      this.signalPerformance.volume.total++;
      if (Math.abs(priceChange) > 0.3) { // Significant move followed spike
        this.signalPerformance.volume.correct++;
      }
    }

    // 3. ROC Performance (Panic trigger)
    if (signals.roc.isPanic) {
      this.signalPerformance.roc.total++;
      if (priceChange < -0.5) { // ROC panic correctly predicted dump
        this.signalPerformance.roc.correct++;
      }
    }
  }
  ...
  private calculateMetrics(): FastMetrics {
    ...
    const takerFees = this.trades.reduce((sum, t) => sum + (t.sizeUsd * 0.0005), 0)
    const fundingFees = this.trades.reduce((sum, t) => sum + (t.fees || 0), 0) - takerFees
    const slippageCosts = this.trades.reduce((sum, t) => sum + (t.sizeUsd * 0.0001), 0)
    ...
    // Value at Risk (95% confidence) - simplified historical VaR
    const sortedReturns = [...returns].sort((a, b) => a - b)
    const var95 = sortedReturns.length > 0 
      ? Math.abs(sortedReturns[Math.floor(sortedReturns.length * 0.05)] || 0) * this.config.totalInvestmentUsd
      : 0

    // Signal accuracy (dummy values for now since we need to track them during loop)
    const signalAccuracy = {
      parabolicSAR: (this.signalPerformance && this.signalPerformance.parabolicSAR.total > 0) 
        ? (this.signalPerformance.parabolicSAR.correct / this.signalPerformance.parabolicSAR.total) * 100 
        : 0,
      volumeSpike: (this.signalPerformance && this.signalPerformance.volume.total > 0) 
        ? (this.signalPerformance.volume.correct / this.signalPerformance.volume.total) * 100 
        : 0,
      roc: (this.signalPerformance && this.signalPerformance.roc.total > 0) 
        ? (this.signalPerformance.roc.correct / this.signalPerformance.roc.total) * 100 
        : 0
    }
    ...
    return {
      ...
      confidenceScore,
      totalFees: takerFees + fundingFees + slippageCosts,
      feeBreakdown: {
        takerFees,
        fundingFees,
        slippageCosts
      ...
      riskMetrics: {
        var95,
        largestConsecutiveLoss: maxConsecutiveLoss
      },
      signalAccuracy
    }
  }
